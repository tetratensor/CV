...
## CrowdWorks, 大規模認証プラットフォームリプレイス（2022 年 8 月〜2023 年 3 月）
...
## [Smart Hospital](https://www.smarthp.co.jp/), 医療メディアフロントエンドリプレイス（2021〜2022）
...
## CrowdWorks, メール送信 API（2021）

### 業務概要

複数アプリケーションで重複していたメール送信ロジックを統一し、一元管理を実現。
...
- メール送信ロジックを一元化、SES から SendGrid に変更して到達率向上
- IP ウォームアップと独自ドメイン設定で大量送信対応
- バウンスメール管理の簡素化
- 後で Lambda の同時実行制限を回避するため App Runner に移行

## CrowdWorks, 郵便番号住所検索 API（2021）

### 業務概要

複数サービスで重複していた郵便番号から住所を検索する処理を統一し、共通 API として提供。
...
- Go + AWS Lambda で KENALL API を呼び出し住所情報を返す API を実装
- 優れた API 設計と低料金で KENALL を採用
- 結果:
  - 郵便番号から住所取得処理の統一
  - 住所マスタメンテナンスの不要化
- GitHub Actions で CI/CD 設定
- 後で Lambda の同時実行制限とコールドスタートを回避するため App Runner 移行を検討

## [Crowdfunding Service](https://fundinno.com/) メッセージ一斉送信改善（2021）
...
- メール送信時間と DB インサート時間の問題を解決
- 既存テーブル構造を活かしつつ一斉送信用関連テーブルを追加
- SendGrid の 1000 件ずつバッチ送信で送信時間を大幅短縮
- Delayed Job で非同期処理化
- 結果:
  - 送信可能件数を 3,000 件から 20,000 件超に拡張
  - ユーザー待ち時間の解消

## [Crowdfunding Service](https://fundinno.com/) 認証プラットフォーム移行（2020〜2021）
...
## [Crowdfunding Service](https://fundinno.com/) コメント取得 API（2020）
...
- OpenAPI スキーマ設計、Rails API 実装、ERB から React SPA 化
- 制約: 高優先度・短期間、テーブル構造変更不可、OpenAPI ルール未確定、ドメイン知識不足
- マイクロサービス化進行中で現場エンジニアと協議して以下方針で実装:
  - コメント表示を SPA 化、スクロール時に新コメント取得
  - ページング対応 API 実装
- スロークエリ回避、React DevTools でパフォーマンス最適化
- 将来のテーブル構造変更に対応する API インターフェース設計
- OpenAPI スキーマ設計ルールを現場エンジニアと共同策定
- 結果:
  - 大量コメントプロジェクトでの表示遅延解消
  - 全コメント表示可能（従来の表示件数上限撤廃）

## Jiitak, EKS 通知サービス（2020）
...
## Jiitak, EC2 から AWS Fargate 移行（2018）
...
## Extlink, 看護師コミュニティサイト全面リプレイス（2017）
...
## Extlink, 大手 EC サイト会員基盤全面リプレイス（2016）
...
- Java 8（API サーバ）, Spring Framework 4, Ruby on Rails 4（管理サイト）
- MySQL 5.6, Couchbase 3（バックエンドキャッシュ）, Redis（メッセージング）
...
- 開発リーダーとして旧 DB から新 DB へのデータ移行計画作成、移行テスト自動化
- システム全体アーキテクチャ決定、新会員基盤 API 設計
- 新会員管理サイト要件定義・設計・開発
- Ansible でサーバ構成管理、API 開発
- 結果:
  - DB 構造一新で性能大幅向上、急激な会員数増加に対応
  - AWS 性能試験環境構築でミドルウェア性能試験を気軽に実行
- 大規模案件で技術負債多く困難だったが完遂
- 学んだこと:
  - リプレイス案件で最も高難易度はデータ移行
  - 少数精鋭チームの重要性（頭数揃えても開発スピード向上しない）
  - 事前技術検証の重要性
...
- Node.js（Express）, Redis（API Gateway キャッシュ）
- PHP 5.4（FuelPHP）※クライアント ID 等管理サイト
...
- 結果:
  - 独自実装 API 認証・認可が一般的な仕組みに置換
  - API アクセス管理でクライアント別機能利用状況が明確化
  - 継続的インテグレーション導入でリリース後不具合大幅減少
- OAuth Provider 実装で OpenID Connect 公式仕様を参照
- RFC・公式ドキュメント等 1 次情報（主に英語）を読む重要性を再認識

## Extlink, 社内共通デプロイツール改修（2014）
...
## Extlink, 大手 EC サイトソーシャルログイン導入（2013）

### 業務概要

大手 EC サイトにソーシャルログイン機能導入。初期リリースで Facebook と Google に対応。

### 利用した技術

- PHP 5.3, Zend Framework 1, Facebook API, Google API

### プロジェクトの規模

3 名: 自分（開発リーダー）、開発者 1 名、ディレクター 1 名

### 自分の役割・主な成果

- 要件定義、設計全般、プログラム実装
- リリース後 1 ヶ月で会員登録数 50 万人増加
- 重要機能のため安全な OAuth クライアント実装方法を調査・設計・実装
- OAuth・OpenID Connect 基礎理解

## Extlink, 大手 EC サイト個人情報 API 高速化（2013）
...
- PHP 5.3, Zend Framework 1, MySQL 5.1/5.5
...
- memcached キャッシュ導入による処理高速化
- MySQL クエリチューニングによる SQL 実行速度向上
- MySQL サーバーバージョンアップ
- 10 年近く前のレガシーテーブル構造制約下で SQL 組み立て工夫
- 平均応答速度 400ms → 150ms 改善
- 性能実用レベル達成で以降実際プロダクトで利用開始

## Extlink, 大手 EC サイト個人情報 API 開発（2013）

### 業務概要

ユーザーデータアクセス機能を WebAPI 化して分離。

### 利用した技術

- PHP 5.3, Zend Framework 1, MySQL 5.1

### プロジェクトの規模

4 名: エンジニア 3 名（自分含む）、ディレクター 1 名

### 自分の役割・主な成果

- API 開発・ユニットテスト作成
- API 内部設計（外部設計は先輩エンジニア担当）
- 正社員エンジニア初案件
- ユニットテスト未記述クラスに積極的にテスト追加、テスト自動化に貢献
- コードレビューで変更に強い設計手法、Git 運用ルール習得
